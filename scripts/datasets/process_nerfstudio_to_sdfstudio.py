...
def main(args):
    ...
    """
    output_images = Path(args.images_output)
    input_images = Path(args.images_input)
    transforms_json = Path(args.transforms_json)
    output_transforms = Path(args.output_transforms)
    output_images.mkdir(parents=True, exist_ok=True)
    cam_params = json.load(open(transforms_json))
    ...
    for frame in frames:  # load intrinsics from polycam
        ...
        img_path = input_images / file_path.name
    ...
    for idx, (valid, pose, image_path) in enumerate(tqdm(zip(valid_poses, poses, image_paths))):
        ...
        # save rgb image
        out_img_path = output_images / f"{out_index:06d}_rgb.png"
        img = Image.open(image_path)
        img_tensor = rgb_trans(img)
        img_tensor.save(out_img_path)
        rgb_path = str(out_img_path.relative_to(output_images))
    ...
    if args.mono_prior:
        ...
        os.system(
            ...
            --img_path {output_images} --output_path {output_images} \
            --task depth"
        )
        ...
        """
    print(f"Done! The processed data has been saved in {output_images}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--images", dest="images_input", required=True, help="path to nerfstudio data directory")
    parser.add_argument(
        "--transforms-json", dest="transforms_json", required=True, help="path to nerfstudio data directory"
    )
    parser.add_argument("--output-images", dest="images_output", required=True, help="path to output data directory")
    parser.add_argument(
        "--output-transforms", dest="output_transforms", required=True, help="path to nerfstudio data directory"
    )
